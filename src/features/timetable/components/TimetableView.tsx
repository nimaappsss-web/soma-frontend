...
export const TimetableView = () => {
  ...
  const currentType = current?.schoolType ? effectiveSchoolType(current.schoolType) : undefined;
  const config = currentType ? configs[currentType] : undefined;

  const openBuilder = () => {
    if (currentType && !config) {
      toast.error(`No configuration set for ${schoolTypeLabel(currentType)}`);
      navigate(`/admin/timetable/configs?type=${encodeURIComponent(currentType)}&edit=1`);
      return;
    }
    setBuilding(true);
  };
      ...
      {isLoading ? (
        ...
        error ? (
          <div className="mt-10 rounded-xl border border-input p-8 text-center text-sm text-placeholder">
            Could not load the timetable. Check your connection and try again.
          </div>
        ) : (
        ...
        <div className="mt-6 space-y-6">
          ...
          <div className="flex items-center justify-between text-xs text-placeholder">
            ...
            <Button type="button" variant="outline" size="sm" onClick={openBuilder}>
